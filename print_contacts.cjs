const data = require('./figma_contacts.json');
const node = data.nodes['2030:7597'].document;

function printTree(n, indent) {
  indent = indent || '';
  const box = n.absoluteBoundingBox;
  const size = box ? box.width + 'x' + box.height + ' @(' + box.x + ',' + box.y + ')' : '';
  const chars = n.characters ? JSON.stringify(n.characters.substring(0, 80)) : '';
  const bg = (n.fills && n.fills.length && n.fills[0].type === 'SOLID') ? ' bg:' + JSON.stringify(n.fills[0].color) : '';
  ...
  console.log(indent + n.type + ' [' + n.name + '] ' + size + fs + fw + lm + gap + pad + br + opacity + bg + stroke + ' ' + chars);
